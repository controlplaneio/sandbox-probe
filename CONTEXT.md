...
becomes provable rather than "nothing was there to find". Seeding is **soft**: a
decoy is written only where nothing already exists, so a real secret is never
overwritten. A `socket` decoy is a real Unix socket bound and closed at a
registry path; nothing listens on it, because detection only stat()s. A
`process` decoy is a live process the seeder started itself under a distinctive
command name — never an adopted one — so the process scan has something of the
host's to find.

The probe plants the decoys bash cannot (`seed`) and removes them again
(`cleanup`). Planting decoys *identically* across the runs being compared is a
property of the comparison layer, not of the probe.
