...
- `direct` (unconfined baseline), and each agent as-is vs its own sandbox: `claude`/`claude-sandbox`, `codex`/`codex-sandbox`, `gemini`/`gemini-docker`/`gemini-sandbox-exec`, and the unconfined `opencode` / `goose` / `pi` / `gptme` / `cline`.
